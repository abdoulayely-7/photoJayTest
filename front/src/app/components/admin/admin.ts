...
export class AdminComponent implements OnInit {
  ...
  approveProduct(product: Product): void {
    Swal.fire({
      ...
      text: `Êtes-vous sûr de vouloir approuver "${product.title}" ?`,
      icon: 'question',
      ...
      if (result.isConfirmed) {
        this.apiService.approveProduct(product.id).subscribe({
          next: (approvedProduct) => {
            // Update the product in the list
            const updatedProducts = this.products().map(p =>
              p.id === product.id ? approvedProduct : p
            );
            this.products.set(updatedProducts);
            this.closeModal();
            Swal.fire({
              title: 'Succès!',
              text: 'Produit approuvé avec succès',
              icon: 'success',
              timer: 2000,
  ...
  rejectProduct(product: Product): void {
    Swal.fire({
      ...
      inputPlaceholder: 'Expliquez pourquoi ce produit est rejeté...',
      inputAttributes: {
      ...
      if (result.isConfirmed) {
        ...
        this.apiService.rejectProduct(product.id, reason).subscribe({
          next: (rejectedProduct) => {
            // Update the product in the list
            const updatedProducts = this.products().map(p =>
              p.id === product.id ? rejectedProduct : p
            );
            this.products.set(updatedProducts);
            this.closeModal();
            Swal.fire({
              title: 'Succès!',
              text: 'Produit rejeté avec succès',
              icon: 'success',
              timer: 2000,
              showConfirmButton: false
            });
